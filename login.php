...
if (isset($_REQUEST['login'])) {
  ...
  if (!empty($email) && !empty($pass)) {
    $sql = "SELECT * FROM user where usemail='$email' && uspass='$pass'";
    ...
    if ($row) {
      ...
      $error = "<p class='alert alert-warning'>Электронная почта или пароль не совпадают!</p> ";
    }
  } else {
    ...
    <form class="space-y-6" method="post">
      ...
      <div>
        <div class="flex items-center justify-between">
          ...
          <div class="text-sm">
            <a href="forgotpassword.php" class="font-semibold text-indigo-600 hover:text-indigo-500">Забыли пароль?</a>
          </div>
        </div>
        <div class="mt-2">
